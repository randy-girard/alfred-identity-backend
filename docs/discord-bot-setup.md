...
1. Open [Discord Developer Portal](https://discord.com/developers/applications) → **New Application** (e.g. `Alfred Identity`).
...
11. Optional web admin: set `DISCORD_CLIENT_SECRET`, `WEB_ENABLED=true`, `WEB_PUBLIC_URL`, and add OAuth2 redirect `{WEB_PUBLIC_URL}/admin/oauth/callback`. See [web-admin.md](web-admin.md).
...
EQ accounts, aliases, tags, characters, private Local → Share access, and user access are managed in the **desktop GUI** or **web admin**.
...
2. Users run `/alfred-identity-sso create` and paste the token into the desktop app
